...
def generate_hls_or_dash(input_file, output_formats, is_hls=True):
    ...
    for output_format in output_formats:
        ...
        for resolution in resolutions:
            ...
            output_file = os.path.join(output_directory, f"{base_file_name}_{resolution['resolution']}.{format}")
...
def generate_hls_manifest(input_file, output_formats):
    ...
    with open(manifest_file, 'w') as f:
        f.write("#EXTM3U\n")
        for resolution in resolutions:
            resolution_file = os.path.join(output_directory, f"{os.path.splitext(os.path.basename(input_file))[0]}_{resolution['resolution']}.{output_format['format']}")
