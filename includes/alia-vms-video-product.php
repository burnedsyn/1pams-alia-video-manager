...
require_once ALIA_VMS_PLUGIN_PATH . '/includes/alia-vms-video-transcoder.php';
require_once ALIA_VMS_PLUGIN_PATH . '/includes/alia-vms-upcloud.php';
...
{
  public function handle_vimeo_import($files,  $titles, $descriptions, $prices = 19)
  {
    $file = $files; //vimeo link

    $title = urldecode($titles);
    $decodedtitle = html_entity_decode($title);
    ...
    $test = $upload_dir['basedir'] . '/video_upload/' . $decodedtitle;
    ...
    chmod($DASH_dir, 0775);
    file_put_contents($base_dir.'/'.$decodedtitle.'.mp4', fopen($file, 'r'));
    $filePath=$base_dir.$decodedtitle.'.mp4';
    $transcoder = new Alia_VMS_Transcoder();
    $transcoded = $transcoder->transcode_vimeo_import($filePath, $product_id);

    $returnData=[
      'product_id'=>$product_id,
      'file_Path' => $filePath,
      'outputlog' => $transcoded,
      ];
